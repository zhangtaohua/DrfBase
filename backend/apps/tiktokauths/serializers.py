...
class TiktokAppleLoginSerializer(TokenObtainPairSerializer):

  @classmethod
  def get_token(cls, user):
    token = super(TiktokAccountSerializer, cls).get_token(user)
    data = {}
    data["openid"] = user.tiktokusers.applet_openid
    data["userid"] = user.id
